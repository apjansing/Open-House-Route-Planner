...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item The directions matrix (see slide \ref{directions matrix}) were used to manipulate the data into a form that would be useful for the OpenHouseGraph.
      \item And the Esri Flask App (see slide \ref{Esri Flask App}) gathered data from the Esri Developer API to geocode addresses to locations and fetched directions between two points so we could see how long it would take to travel between houses.
    \end{itemize} \pause
    \item[Load] - MongoOps (see slide \ref{MongoOps}) performed several roles:
      \begin{itemize}
        ...
        \item and requesting information from the Esri Flask App if the data did not exists in its database.
      \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  And all of this had to be supported by an underlying infrastructure.\pause \\ 
...
\begin{frame}[fragile]
  \frametitle{Definitions}
  \begin{itemize}
    \item \emph{directions matrix}\label{directions matrix} - an array of JSONs containing vertex and edge information. While not strictly a matrix, it supplies the information to populate a matrix of travel times. While a matrix is no longer used, it is a good mental image to have when thinking about the OpenHouseGraph.
    \begin{figure}
      \begin{lstlisting}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \pause\item \href{https://github.com/apjansing/Open-House-Route-Planner/blob/master/backend/docker/persistence/pyspark/ICSParser.py}{ICSParser}\label{ICSParser} - Class for parsing ICS files to JSON to be geocoded.
...
\begin{frame}[fragile]
  ...
  \begin{minipage}[t]{1.1\linewidth-1.2em} % some separation
    ...
    \end{multicols}
  \end{minipage}
    \caption{Shortend ICS files.}\label{short-ics}
  \end{figure}
\end{frame}
...
[{'ID': 0, 'address': location0 (Clinton),
 ...
 {'ID': 6, 'address': location6 (Oneida),
  ...
  'end': 720, 'start': 600}]
    \end{lstlisting}
    \caption{Shortend Parsed ICS files.}\label{short-ics}
  \end{figure}
\end{frame}
